...
$item_price_SQL = "SELECT item_price FROM item WHERE item_id = $itemID";
...
$transactionTotal_SQL = "SELECT transaction_id,transaction_total FROM transaction WHERE customer_id='$userID' AND transaction_date = 'NULL' ";
...
$delete_Operation_SQL = "DELETE FROM transaction_item WHERE item_id='$itemID'";


$subtract_From_Total_SQL = "UPDATE transaction
...
if (DB()->query("$delete_Operation_SQL") && DB()->query("$subtract_From_Total_SQL")) {
    ...
    if($newTotal == 0){
        $delete_transaction_SQL = "DELETE FROM transaction WHERE transaction_id = '$transID';";
